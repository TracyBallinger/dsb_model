...
fastqdir=/exports/eddie/scratch/tballing/ncbi/fastq
refgen=/exports/igmm/software/pkg/el7/apps/bcbio/share2/genomes/Hsapiens/GRCh37/bwa/GRCh37.fa  # Reference genome fasta file with bwa index
patfile=/home/tballing/bioinfsvice/DSB_model/BLISS/scripts/silvano_scripts/patterns/example.txt  # this is the pattern file: ../pattern/example.txt
quality=30	 # mapping quality - they use 30 in the published BLISS paper
numproc=3			# number of threads to be used
scandir=/home/tballing/NextGenResources/software/scan_for_matches/scan_for_matches
...
tmpdir=/exports/eddie/scratch/tballing/bliss
mkdir -p $tmpdir
...
echo "cat $tmpdir/r1.fa | parallel --tmpdir $tmpdir --block 100M -k --pipe -L 2 "$scandir/scan_for_matches $patfile - " > $tmpdir/filtered.r1.fa"
cat $tmpdir/r1.fa | parallel --tmpdir $tmpdir --block 100M -k --pipe -L 2 "$scandir/scan_for_matches $patfile - " > $tmpdir/filtered.r1.fa
cat $tmpdir/filtered.r1.fa | cut -d':' -f1 | tr '>' '@' | paste - - | awk '{print $1,$NF}' | LC_ALL=C sort --temporary-directory=$tmpdir -k1,1 > $tmpdir/ID_genomic
LC_ALL=C join $tmpdir/ID_genomic $tmpdir/r1oneline.fq | awk '{print $1,$2,$6,substr($9, length($9)-length($2)+1, length($5))}' | tr " " "\n" > $tmpdir/r1.2b.aln.fq

###################################################################
# If it's paired end, then get the matching second read for the filtered ones.  
if [ ! -z $r2 ]; then
	cat $r2 | paste - - - - | LC_ALL=C sort --temporary-directory=$tmpdir -k1,1 > $tmpdir/r2oneline.fq
	# Filter for the read1s with the pattern  
	LC_ALL=C join $tmpdir/ID_genomic $tmpdir/r2oneline.fq | cut -d' ' -f 1,5,6,9 | tr " " "\n" > $tmpdir/r2.2b.aln.fq 
fi 
fi 
...
centbed=/home/tballing/bioinfsvice/DSB_model/BLISS/scripts/hg19-telomere-centromere-telomere.bed
blacklist=/home/tballing/bioinfsvice/DSB_model/BLISS/scripts/consensusBlacklist.bed
...
pbin=/home/tballing/bioinfsvice/DSB_model/BLISS/scripts/silvano_scripts/python
python $pbin/umi_filtering_tjb.py $tmpdir/$experiment.q30.umi.csv $tmpdir/umi_filt1.txt
